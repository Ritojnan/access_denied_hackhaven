...
//   const [ userState, setUserState ] = useContext(UserContext);
  ...
  const arrayToDialogueString = (array) => {
    let dialogueString = '';
    array.forEach((obj, index) => {
      for (const key in obj) {
        if (key === 'user') {
          dialogueString += `${obj[key]}: `;
        } else if (key === 'meetData') {
          dialogueString += `${obj[key]}\n`;
        }
      }
    });
  ...
  // Convert array to dialogue string
  ...
  useEffect(() => {
    const fetchTranscripts = async () => {
      try {
        ...
        setTransData(transcriptsData.join(","));

        const dialogueString = arrayToDialogueString(transcriptsData);
  
        // Output the dialogue string
  ...
  useEffect(() => {
    ...
    const evaluationResult =
      random < 0.5 ? " is less than 50%" : " is greater than or equal to 50%";
  ...
  async function generateMinutes() {
    const options = {
      ...
      data: {
        providers: "openai",
        text: `${dialogue} generate after meet timeline for the transcript         
          `,
        temperature: 0.2,
        max_tokens: 1024,
        fallback_providers: "",
  ...
  async function generateText() {
    ...
    const options = {
      ...
      data: {
        providers: "openai",
        text: `This is a dialogue of a meeting :${dialogue} Generate after meet summary for this in 300 words         
          `,
        temperature: 0.2,
        max_tokens: 1024,
        fallback_providers: "",
    ...
    >
      <div className="container grid gap-6 px-4 md:gap-10 md:px-6">
        <div className="flex flex-col gap-4 md:gap-6">
          ...
          <p className="max-w-prose text-gray-400 md:text-lg">{trans}</p>
        </div>
