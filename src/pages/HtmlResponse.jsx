...
const Response = () => {
  ...
  const generateActivities = async () => {
    try {
      ...
      const options = {
        ...
        data: {
          providers: 'openai',
          text: `These are the items detected in procturing: ${detectedObjects.map(object => object.class).join(', ')}. Make aN ALERT message to send to the user on procturing detection in a proffessional meeting as a soft  THAT IT CAUSES DISTRACTION including the list of objects DONT INCLUDE EXTRA OBJECTS.DONT INCLUDE NAMES `,
          temperature: 0.2,
          max_tokens: 1024,
          fallback_providers: '',
        },
  ...
  return (
    <div>
      ...
      {/* Button to generate activities */}
      <button className='bg-blue-400 p-1 rounded-md ml-3 relative' onClick={generateActivities}> Notifications </button>
